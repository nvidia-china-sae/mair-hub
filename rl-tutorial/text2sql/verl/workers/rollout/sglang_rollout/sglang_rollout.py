...
class SGLangRollout(BaseRollout):
    ...
    async def _async_rollout_a_request(
        ...
        **kwargs,
        ...
        # 生成1-128之间的随机数
        # import random
        # random_number = random.randint(1, 128)
        ...
        # log_dir = Path("/apps/verl_multi-turn/verl/verl/log_data/requests")
        # os.makedirs(log_dir, exist_ok=True)

        # # 生成1-512之间的随机数
        # import random
        # random_number = random.randint(1, 512)
        
        # if random_number == 1:
        #     import time
        #     current_time = time.strftime("%Y%m%d_%H%M%S")
        #     log_file_path = log_dir / f"request_{current_time}.txt"
        #     # if len(_req.messages) > 3:
        #     with open(log_file_path, "w", encoding="utf-8") as f:
        #         f.write(f"[SGLangRollout] 当前请求 ID: {_req.request_id} 的消息列表:\n")
        #         for i, msg in enumerate(_req.messages):
        #             role = msg.role
        #             content = msg.content if msg.content else "(无内容)"
        #             tool_calls_info = f", tool_calls: {msg.tool_calls}" if msg.tool_calls else ""
        #             f.write(f"  消息[{i}] - 角色: {role}, 内容: {content}{tool_calls_info}\n")
        ...
        if current_turns >= self.config.multi_turn.max_assistant_turns:
            ...
            if self.config.multi_turn.final_summary:
                ...
                if last_assistant_message is not None and "<answer>" not in last_assistant_message.content:
                    ...
                    # Make the final prompt configurable through config
                    final_prompt = self.config.multi_turn.summary_prompt
                    _req.add_user_message(self.processing_class, final_prompt)
                    
                    # Check if we still have room in the context for one more generation
                    # if len(_req.get_generation_prompt_ids(self.processing_class)) + 1 < self.config.max_model_len:
                    try:
                        ...
                        # Update finish reason based on the final generation
                        finish_reason_type = final_finish_reason
                        
                        # Log the final generated content for debugging
                        if logger.isEnabledFor(logging.DEBUG):
                            logger.debug(f"Request {_req.request_id}: Final generated content: {final_content[:200]}...")
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        for req in sorted_output_req_list:
            ...
            assistant_turns = sum(1 for msg in req.messages if msg.role == "assistant")
            
            # # 当 assistant_turns >= 2 时，保存 error_message 数据到文件
            # if assistant_turns >= 2:
            #     import os
            #     from pathlib import Path
            #     import time
                
            #     log_dir = Path("/apps/verl_multi-turn/verl/verl/log_data/error_messages")
            #     os.makedirs(log_dir, exist_ok=True)
                
            #     current_time = time.strftime("%Y%m%d_%H%M%S_%f")[:-3]  # 添加毫秒精度
            #     log_file_path = log_dir / f"error_message_{current_time}_{req.request_id}.txt"
                
            #     with open(log_file_path, "w", encoding="utf-8") as f:
            #         f.write(f"[SGLangRollout] 请求 ID: {req.request_id}\n")
            #         f.write(f"Assistant 轮次数: {assistant_turns}\n")
            #         f.write(f"时间戳: {current_time}\n")
            #         f.write("=" * 50 + "\n")
            #         f.write("Error Message:\n")
            #         f.write(error_message)
            #         f.write("\n" + "=" * 50 + "\n")
                
            #     logger.info(f"已保存 assistant_turns >= 2 的 error_message 数据到文件: {log_file_path}")
            
            turn_stats.append(assistant_turns)
        ...
        return DataProto(
            ...
            non_tensor_batch={
                ...
                "turn_stats": np.array(turn_stats),
