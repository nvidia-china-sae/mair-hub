...
class AsyncRolloutRequest(BaseModel):
    ...
    def _get_prompt_diffs(
        ...
        # 将 full prompt 和 current prompt 写入文件以便调试
        ...
        #     log_dir = Path("/apps/verl_multi-turn/verl/verl/log_data/prompts")
        #     os.makedirs(log_dir, exist_ok=True)

        #     current_time = time.strftime("%Y%m%d_%H%M%S")
        #     log_file_path = log_dir / f"prompt_diff_{current_time}.txt"
            
        #     with open(log_file_path, "w", encoding="utf-8") as f:
        #         f.write(f"[PromptDiff] 完整提示和当前提示的比较:\n")
        #         f.write(f"完整提示长度: {len(full_prompt)}, 当前提示长度: {len(current_prompt)}\n\n")
        #         f.write("===== 完整提示 =====\n")
        #         f.write(full_prompt)
        #         f.write("\n\n===== 当前提示 =====\n")
