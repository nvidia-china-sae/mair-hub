...
def _compute_multi_turn_stats(batch: DataProto) -> Dict[str, Any]:
    """
    Compute multi-turn conversation statistics from a batch of data.

    This function calculates statistics about the number of conversation turns
    for multi-turn dialogue training.

    Args:
        batch: A DataProto object containing batch data with turn statistics in non_tensor_batch.

    Returns:
        A dictionary containing:
            - num_turn/min: Minimum number of turns in the batch
            - num_turn/max: Maximum number of turns in the batch  
            - num_turn/mean: Average number of turns in the batch
            - num_turn/total_samples: Total number of samples in the batch

    Note:
        Returns empty dict if turn_stats is not available in the batch.
    """
    if not hasattr(batch, 'non_tensor_batch') or batch.non_tensor_batch is None:
        return {}
    
    if "turn_stats" not in batch.non_tensor_batch:
        return {}
    
    try:
        turn_stats = batch.non_tensor_batch["turn_stats"]
        
        # Convert to numpy array if it's not already
        if not isinstance(turn_stats, np.ndarray):
        ...
        # Filter out any invalid values (negative or None)
        valid_turns = turn_stats[turn_stats >= 0]
        
        if len(valid_turns) == 0:
            return {}
        
        return {
            "num_turn/min": int(np.min(valid_turns)),
            "num_turn/max": int(np.max(valid_turns)),
            "num_turn/mean": float(np.mean(valid_turns)),
            "num_turn/total_samples": len(turn_stats),
        }
        ...
        logger.warning(f"Failed to compute multi-turn stats: {e}")
        return {}
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    multi_turn_stats = _compute_multi_turn_stats(batch)
    metrics.update(multi_turn_stats)
