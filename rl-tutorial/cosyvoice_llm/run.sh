...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  ...
  python3 -m verl.trainer.main_ppo \
      ...
      trainer.experiment_name='whisper_turbo_per_aishell3_reward_tts' \
...
if [ $stage -le 21 ] && [ $stop_stage -ge 21 ]; then
  log "stage 21: Prime reward manager"
  ...
  python3 -m verl.trainer.main_ppo \
      ...
      custom_reward_function.name=compute_score \
      trainer.project_name='llasa_tts_grpo' \
      trainer.experiment_name='whisper_turbo_per_aishell3_reward_tts_prime' \
...
llm_path=/workspace/slam/mair-hub/rl-tutorial/cosyvoice_llm/checkpoints/llasa_tts_grpo/whisper_turbo_per_aishell3_reward_tts/global_step_3000
if [ $stage -le 5 ] && [ $stop_stage -ge 5 ]; then
  log "stage 5: merge the model"

  python  /workspace/rl/verl/scripts/legacy_model_merger.py merge \
      --backend fsdp \
...
if [ $stage -le 6 ] && [ $stop_stage -ge 6 ]; then
  log "stage 6: Test the model"
  dataset=zero_shot_zh
  output_dir=./outputs_rl_aishell3_step3000_${dataset}_jit_trt_fp16_reward_tts
