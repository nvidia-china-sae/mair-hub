...
if __name__ == '__main__':
    ...
    dataset = ds['test'].shuffle(seed=42).select(range(100))
    dataset = dataset.map(function=make_map_fn('test', data_source, system), with_indices=True)
    dataset.to_parquet(os.path.join(args.local_dir, 'math500.parquet'))
