...
if __name__ == '__main__':
    ...
    dataset = ds['train'].map(function=make_map_fn('train', data_source, system), with_indices=True, num_proc=8)
    print(f"before filter: {len(dataset)}")
    dataset = dataset.filter(lambda x:  x['prompt'][0]['content'].count("<image") == 1, num_proc=8)
    print(f"after filter: {len(dataset)}")
    dataset.to_parquet(os.path.join(args.local_dir, 'geo3k.parquet'))
