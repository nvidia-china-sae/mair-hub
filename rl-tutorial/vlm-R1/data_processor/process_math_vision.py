...
def make_map_fn(split, processor, data_source, system):
    ...
    def process_fn(example, idx):
        ...
        image_num = question.count("<image")
        if image_num > 1:
            answer = None
        question = "<image>\n" + question.replace("<image1>", "")
        prompt = question
...
if __name__ == '__main__':
    ...
    data_source = 'MathLLMs/MathVision'
    ...
    dataset = ds['test'].map(function=make_map_fn('test', processor, data_source, system), with_indices=True, num_proc=8)
    # filter out the data
    ...
    dataset.to_parquet(os.path.join(args.local_dir, 'math-vision.parquet'))
